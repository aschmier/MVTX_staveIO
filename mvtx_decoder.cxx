...
void readMSD(vector<uint8_t> sensor_data){
  ...
  for(int i = 0; i < sensor_data.size(); i++){
    ...
    if((sensor_data.at(i)>>4) == 0xb){
      ...
      cout << "Readout Flags: " << (unsigned int)(sensor_data.at(i) & 0xf) << " ";
      break;
    }
    if((sensor_data.at(i)>>5) == 0x6){
      cout << "Region Header:    " << (unsigned int)sensor_data.at(i) << "     -> ";
      cout << "Region ID:     " << (unsigned int)(sensor_data.at(i) & 0x1f) << " ";
    }
    if((sensor_data.at(i)>>4) == 0xa){
      ...
      cout << "Bunch Counter: " << (unsigned int)sensor_data.at(i+1) << " ";
    ...
    if((sensor_data.at(i)>>6) == 0x1){
      ...
      cout << "Address:       " << (unsigned int)(dsBin & 0x3ff) << " ";
      if(i < sensor_data.size()-2) i++;
    }
    if((sensor_data.at(i)>>6) == 0x0){
      ...
      cout << "Hit Map: " << (unsigned int)(dlBin & 0x7f) << " ";
...
void wordHandler(int word_number, vector<uint8_t> vec, bool& MSDpacket, vector<uint8_t>& lane0data, vector<uint8_t>& lane1data, vector<uint8_t>& lane2data, bool isFDH, vector<uint8_t>& MSDword){
  if(!MSDpacket) cout << "   | ";
  cout << hex;
  ...
  bool isMSD = ((vec[9]>>5) == 0x01);

  //Felix data header
  if(isFDH){ cout << "//////////   Felix Data Header. Reading from lane " << (unsigned int)vec[8] << "   //////////" << endl; isFDH = false; }
  ...
  if(isRDH && word_number == 1){
    ...
    cout << "gtm_bco: " << (unsigned int)gtm_bco;
  }
  ...
  if(isCDW){
    ...
    cout << "cdw_user_fields: " << (unsigned int)cdw_user_fields;
  }
  ...
  if(isMSD){
    ...
    if(MSDpacket && MSDword.size()){
      for(int i = 0; i < vec.size(); i++){
        if(vec[0] == MSDword.at(0)) lane0data.push_back(vec.at(i));
        if(MSDword.size()>1){ if(vec[0] == MSDword.at(1)) lane1data.push_back(vec.at(i)); }
        if(MSDword.size()>2){ if(vec[0] == MSDword.at(2)) lane2data.push_back(vec.at(i)); }
      }
    }

    for(int i = 0; i < vec.size(); i++){
      if(vec.at(i) & 0xb0 && word_number == 2) MSDpacket = false;
    }
    if(chipEmpty) MSDpacket = false;
  }
/*
  // Felix data header
  if(isFDH){
...
void mvtx_decoder(string inputfile, int sensorIn){
  ...
  if (binFile.is_open())
  {
    ...
    while (binFile.good()) {
        ...
        */
        ...
        for(int word_number = 1; word_number < vBuf.size()+1; word_number++){
          ...
          if(word_number % 10 == 0){
            ...
            for(int i = 0; i < eightbitVec.size(); i++) if(eightbitVec.at(i) != 0) goodLine = true;
            if(!goodLine){eightbitVec.clear(); continue;}
            else goodLine = false;
            // print the line
            if(!MSDpacket) for(int i = eightbitVec.size()-1; i > -1; i--){cout.width(2); cout << hex << (unsigned int)(eightbitVec.at(i)) << " ";}
            // decode
            wordHandler(word_number/10-1, eightbitVec, MSDpacket, lane0data, lane1data, lane2data, isFDH, MSDword);
            eightbitVec.clear();
            if(!MSDpacket) cout << endl;
          }
        }
      }
      if(!MSDpacket && lane0data.size() != 0){ cout << endl << "Lane 0:" << endl; readMSD(lane0data); cout << endl << endl; }
      ...
      if(!MSDpacket && lane0data.size() != 0){cout << endl; lane0data.clear(); lane1data.clear(); lane2data.clear(); MSDword.clear();}
    }
    binFile.close();
  }
